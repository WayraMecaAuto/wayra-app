...
export default function ContabilidadTorniRepuestosPage() {
  ...
  const fetchContabilidad = async () => {
    ...
    try {
      const response = await fetch(
        `/api/contabilidad/torni-repuestos?mes=${mes}&año=${año}`
      );
      if (response.ok) {
  ...
  const eliminarEgreso = async (id: string) => {
    ...
    try {
      const response = await fetch(
        `/api/contabilidad/torni-repuestos?id=${id}`,
        {
          method: "DELETE",
        }
      );
  ...
  const calcularTotales = () => {
    ...
    const totalCostos = ingresos.reduce(
    ...
    <div className="space-y-6 p-4 sm:p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card className="bg-gradient-to-br from-purple-50 to-purple-100 border-purple-200">
          ...
          <CardContent>
            <div
              className={`text-3xl font-bold ${totales.totalUtilidad >= 0 ? "text-purple-800" : "text-red-800"}`}
            >
                  ...
                  filteredIngresos.map((ingreso, index) => (
                    ...
                    >
                      ...
                      <td className="py-3 px-4 text-right font-bold text-green-600">
                        $
                        {(
                          ingreso.precioVenta * ingreso.cantidad
                        ).toLocaleString()}
                      </td>
                      <td className="py-3 px-4 text-right font-bold text-green-600">
