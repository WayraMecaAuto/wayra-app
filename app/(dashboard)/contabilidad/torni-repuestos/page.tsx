...
export default function ContabilidadTorniRepuestosPage() {
  ...
  const eliminarEgreso = async (id: string) => {
    ...
    try {
      const response = await fetch(`/api/contabilidad/torni-repuestos?id=${id}`, { method: "DELETE" });
      if (response.ok) {
  ...
  const filteredIngresos = ingresos.filter(
    (i) =>
      ...
      i.categoria.toLowerCase().includes(searchIngresos.toLowerCase()) ||
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto space-y-6">
        {/* Header */}
        <motion.div variants={itemVariants} className="bg-gradient-to-r from-purple-600 to-purple-800 rounded-2xl p-6 shadow-xl text-white">
          <div className="flex flex-col sm:flex-row items-center gap-4">
            <div className="w-14 h-14 bg-white/25 rounded-xl flex items-center justify-center backdrop-blur-sm">
              <Image src="/images/TorniRepuestos.png" alt="TorniRepuestos" width={38} height={38} className="object-contain" />
            </div>
            <div className="text-center sm:text-left">
              <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Contabilidad TorniRepuestos</h1>
              <p className="text-purple-100 text-sm mt-1">Ventas directas de productos</p>
            </div>
          </div>
        </motion.div>
              ...
              return (
                <>
                  ...
                  >
                    ...
                    <td className="py-3 px-4">{categoriaBadge}</td>
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-slate-50 sticky top-0 z-10 hidden sm:table-header-group">
                      <tr>
                        ...
                        <th className="text-left py-3 px-4 font-medium text-slate-700">{table.hasAdd ? "Concepto" : "Categoría"}</th>
                        {table.hasAdd ? null : (
                          <>
...
function EgresoModal({ isOpen, onClose, onSuccess }: { isOpen: boolean; onClose: () => void; onSuccess: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/contabilidad/torni-repuestos`, {
        method: "POST",
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Agregar Egreso">
      ...
      >
        <div>
          ...
          <Input value={formData.descripcion} onChange={e => setFormData({ ...formData, descripcion: e.target.value })} placeholder="Ej: Compra de herramientas" required className="rounded-xl" />
        </div>
        <div>
