...
import { Plus, Search, CreditCard as Edit, Trash2, Package, TriangleAlert as AlertTriangle, ChartBar as BarChart3, Camera, ArrowUpDown, Eye, ListFilter as Filter, Settings } from 'lucide-react'
...
export default function FiltrosPage() {
  ...
  const fetchProducts = async () => {
    try {
      const response = await fetch('/api/productos?tipo=TORNI_REPUESTO&categoria=FILTROS')
      if (response.ok) {
  ...
  const handleBarcodeScanned = async (code: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (product.tipo === 'TORNI_REPUESTO' && product.categoria === 'FILTROS') {
          setSelectedProduct(product)
          setShowMovementForm(true)
          toast.success(`Producto encontrado: ${product.nombre}`)
        } else {
          toast.error('Este producto no pertenece a Filtros')
        }
      } else {
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6 md:p-8 lg:p-10 animate-fade-in">
      {/* Header */}
      <div className="bg-gradient-to-r from-teal-500 via-teal-600 to-teal-700 rounded-2xl p-4 sm:p-6 md:p-8 text-white shadow-2xl transition-all duration-300 hover:shadow-3xl">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold mb-2">Filtros Automotrices</h1>
              <p className="text-teal-100 text-base sm:text-lg">Filtros de aceite, aire, combustible y más</p>
              <div className="flex flex-wrap gap-2 mt-3 text-sm">
                <span className="bg-white/20 px-3 py-1 rounded-full">Margen 25%</span>
          ...
          <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3">
            ...
            {['SUPER_USUARIO', 'ADMIN_TORNI_REPUESTOS'].includes(session?.user?.role || '') && (
              <Button
                ...
                className="bg-white text-teal-600 hover:bg-teal-50 shadow-lg transition-all duration-300 hover:scale-105 w-full sm:w-auto"
              >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        <Card className="bg-gradient-to-br from-teal-50 to-teal-100 border-teal-200 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-teal-700">Total Filtros</CardTitle>
            <Filter className="h-5 w-5 text-teal-600" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl sm:text-3xl font-bold text-teal-800">{stats.total}</div>
            <p className="text-xs text-teal-600 mt-1">Filtros disponibles</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-green-50 to-green-100 border-green-200 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
          ...
          <CardContent>
            ...
            <p className="text-xs text-green-600 mt-1">Inventario filtros</p>
          </CardContent>
        </Card>
      ...
      {/* Search and Actions */}
      <Card className="shadow-xl border-0 bg-gradient-to-r from-white to-teal-50 transition-all duration-300 hover:shadow-2xl">
        <CardContent className="p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0 gap-4">
            <div className="relative flex-1 max-w-full sm:max-w-md">
              <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-5 w-5" />
              <Input
                placeholder="Buscar filtros automotrices..."
                value={searchTerm}
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              {['SUPER_USUARIO', 'ADMIN_TORNI_REPUESTOS'].includes(session?.user?.role || '') && (
                <Button
                  onClick={() => setShowProductForm(true)}
                  className="w-full sm:w-auto bg-teal-600 hover:bg-teal-700 transition-all duration-300 hover:scale-105"
                >
                  <Plus className="h-4 w-4 mr-2" />
                  Nuevo
                </Button>
      ...
      <Card className="shadow-2xl border-0 bg-white transition-all duration-300 hover:shadow-3xl">
        <CardHeader className="bg-gradient-to-r from-teal-500 to-teal-600 text-white p-4 sm:p-6">
          <CardTitle className="text-lg sm:text-xl flex items-center space-x-2">
            ...
            <span>Filtros Automotrices ({filteredProducts.length})</span>
          </CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          {/* Desktop Table View */}
          <div className="hidden md:block overflow-y-auto max-h-[60vh] scrollbar-thin scrollbar-thumb-teal-300 scrollbar-track-gray-100">
            <table className="w-full table-fixed">
              <thead className="bg-gradient-to-r from-gray-50 to-teal-50 sticky top-0 z-10">
                <tr>
              ...
              <tbody>
                {filteredProducts.map((product, index) => (
                  <tr 
                    key={product.id} 
                    className={`border-b border-gray-100 hover:bg-gradient-to-r hover:from-teal-25 hover:to-teal-50 transition-all duration-200 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-25'}`}
                  >
                    <td className="py-4 px-4 sm:px-6">
                      <div className="flex items-center space-x-3">
                        <div className="w-10 h-10 bg-gradient-to-br from-teal-100 to-teal-200 rounded-xl flex items-center justify-center transition-transform duration-300 hover:rotate-12">
                          <Filter className="h-5 w-5 text-teal-600" />
                        </div>
                    ...
                    <td className="py-4 px-4 sm:px-6">
                      <div className="flex items-center space-x-2">
                        ...
                        <Button
                          ...
                          className="h-8 w-8 p-0 flex items-center justify-center hover:bg-teal-100 hover:border-teal-300 transition-all duration-300 hover:scale-105"
          ...
          {/* Mobile Card View */}
          <div className="md:hidden space-y-4 p-4 overflow-y-auto max-h-[60vh] scrollbar-thin scrollbar-thumb-teal-300 scrollbar-track-gray-100">
                ...
                <CardContent className="p-4">
                  <div className="space-y-3">
                    <div className="flex items-start space-x-3">
                      <div className="w-10 h-10 bg-gradient-to-br from-teal-100 to-teal-200 rounded-xl flex items-center justify-center flex-shrink-0 transition-transform duration-300 hover:rotate-12">
                        <Filter className="h-5 w-5 text-teal-600" />
                      </div>
          ...
          {filteredProducts.length === 0 && (
            <div className="text-center py-16 animate-fade-in">
              <div className="w-24 h-24 bg-gradient-to-br from-teal-100 to-teal-200 rounded-2xl flex items-center justify-center mx-auto mb-4 transition-transform duration-300 hover:scale-105">
                <Filter className="h-12 w-12 text-teal-500" />
              </div>
              <div className="text-gray-500 text-xl font-medium">No se encontraron filtros</div>
              ...
              {!searchTerm && ['SUPER_USUARIO', 'ADMIN_TORNI_REPUESTOS'].includes(session?.user?.role || '') && (
                <Button
                  onClick={() => setShowProductForm(true)}
                  className="mt-4 bg-teal-600 hover:bg-teal-700 transition-all duration-300 hover:scale-105"
                >
                  <Plus className="h-4 w-4 mr-2" />
