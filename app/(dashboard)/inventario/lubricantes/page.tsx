...
import { Plus, Search, CreditCard as Edit, Trash2, Package, TriangleAlert as AlertTriangle, ChartBar as BarChart3, Camera, ArrowUpDown, Eye, Droplets, Settings } from 'lucide-react'
...
export default function LubricantesPage() {
  ...
  const fetchProducts = async () => {
    try {
      const response = await fetch('/api/productos?tipo=TORNI_REPUESTO&categoria=LUBRICANTES')
      if (response.ok) {
  ...
  const handleBarcodeScanned = async (code: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (product.tipo === 'TORNI_REPUESTO' && product.categoria === 'LUBRICANTES') {
