...
export default function RepuestosPage() {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      const response = await fetch(
        "/api/productos?tipo=TORNI_REPUESTO&categoria=REPUESTOS"
      );
      if (response.ok) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      console.log("🔄 Refrescando repuestos...");
      fetchProducts();
    }, 30000);
  ...
  useEffect(() => {
    const handlePriceUpdate = (event: CustomEvent) => {
      console.log("⚡ Precios actualizados, refrescando repuestos...");
      fetchProducts();
      if (
            ...
            <div className="text-center py-16 animate-fade-in">
              ...
              <p className="text-gray-400 mt-2 max-w-md mx-auto">
                {searchTerm
                  ...
                  : "Agrega el primer repuesto automotriz para comenzar"}
              </p>
              {!searchTerm &&
                ["SUPER_USUARIO", "ADMIN_TORNI_REPUESTOS"].includes(
                  session?.user?.role || ""
