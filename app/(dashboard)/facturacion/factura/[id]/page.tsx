        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6">
          <div className="flex items-center space-x-3 sm:space-x-4">
            ...
            <div className="min-w-0">
              ...
              <p className="text-sm sm:text-base text-gray-600 mt-1">Detalles de la factura</p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6 lg:p-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 sm:gap-6 mb-6 sm:mb-8 pb-6 border-b-2 border-gray-200">
            <div className="flex items-start space-x-3 sm:space-x-4">
              ...
              <div>
                <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900">Wayra</h2>
                <p className="text-xs sm:text-sm text-blue-600 font-semibold mt-1">Mecánica Automotriz</p>
                <p className="text-xs sm:text-sm text-gray-600">NIT: 900123456-7</p>
                <p className="text-xs sm:text-sm text-gray-600">Cel: 317 606 7449</p>
                <p className="text-xs sm:text-sm text-gray-600">info@wayra.com</p>
              </div>
            </div>
            <div className="text-left sm:text-right">
              <div className="inline-flex flex-wrap items-center gap-2 mb-3">
                <span className="text-xl sm:text-2xl lg:text-3xl font-bold text-blue-600">{factura.numeroFactura}</span>
                <span className={`px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium ${estadoBadge.bg} ${estadoBadge.text}`}>
                  {estadoBadge.label}
                </span>
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-8">
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <div className="space-y-2 text-xs sm:text-sm">
                <p className="break-words"><span className="font-medium text-gray-700">Nombre:</span> {factura.cliente.nombre}</p>
                ...
                {factura.cliente.email && (
                  <p className="break-all"><span className="font-medium text-gray-700">Email:</span> {factura.cliente.email}</p>
                )}
                {factura.cliente.direccion && (
                  <p className="break-words"><span className="font-medium text-gray-700">Dirección:</span> {factura.cliente.direccion}</p>
                )}
              </div>
            </div>
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <div className="overflow-x-auto -mx-4 sm:mx-0">
                <table className="w-full text-xs sm:text-sm min-w-[400px]">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="text-left p-2 sm:p-3 font-semibold text-gray-700">Descripción</th>
                      <th className="text-right p-2 sm:p-3 font-semibold text-gray-700 whitespace-nowrap">Precio</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-200">
                    {factura.orden.servicios.map((servicio, idx) => (
                      <tr key={idx}>
                        ...
                        <td className="p-2 sm:p-3 text-right font-medium whitespace-nowrap">${servicio.precio.toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="mb-6">
              ...
              <div className="overflow-x-auto -mx-4 sm:mx-0">
                <table className="w-full text-xs sm:text-sm min-w-[600px]">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="text-left p-2 sm:p-3 font-semibold text-gray-700">Código</th>
                      <th className="text-left p-2 sm:p-3 font-semibold text-gray-700">Descripción</th>
                      <th className="text-center p-2 sm:p-3 font-semibold text-gray-700">Cant.</th>
                      <th className="text-right p-2 sm:p-3 font-semibold text-gray-700 whitespace-nowrap">Precio Unit.</th>
                      <th className="text-right p-2 sm:p-3 font-semibold text-gray-700 whitespace-nowrap">Subtotal</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-200">
                    {factura.orden.detalles.map((detalle, idx) => (
                      <tr key={idx}>
                        <td className="p-2 sm:p-3 text-xs">{detalle.producto.codigo}</td>
                        <td className="p-2 sm:p-3">
                          <div className="break-words">{detalle.producto.nombre}</div>
                          {detalle.producto.aplicaIva && (
                            <span className="text-xs text-blue-600">(+IVA)</span>
                          )}
                        </td>
                        <td className="p-2 sm:p-3 text-center">{detalle.cantidad}</td>
                        <td className="p-2 sm:p-3 text-right whitespace-nowrap">${detalle.precioUnitario.toLocaleString()}</td>
                        <td className="p-2 sm:p-3 text-right font-medium whitespace-nowrap">${detalle.subtotal.toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                      ...
                      <tr key={idx}>
                        <td className="p-2 sm:p-3">
                          ...
                          {repuesto.descripcion && (
                            <div className="text-xs text-gray-500 mt-1 break-words">{repuesto.descripcion}</div>
                          )}
                        </td>
                        <td className="p-2 sm:p-3 text-center">{repuesto.cantidad}</td>
                        <td className="p-2 sm:p-3 text-right whitespace-nowrap">${repuesto.precioUnitario.toLocaleString()}</td>
                        <td className="p-2 sm:p-3 text-right font-medium whitespace-nowrap">${repuesto.subtotal.toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="w-full sm:w-80 space-y-2 sm:space-y-3 bg-gray-50 rounded-lg p-4">
              {factura.orden.manoDeObra > 0 && (
                <div className="flex justify-between text-xs sm:text-sm pb-2 border-b border-gray-200">
                  ...
                  <span className="font-medium whitespace-nowrap">${factura.orden.manoDeObra.toLocaleString()}</span>
                </div>
              )}
              <div className="flex justify-between text-xs sm:text-sm pb-2 border-b border-gray-200">
                <span className="text-gray-600">Subtotal:</span>
                <span className="font-medium whitespace-nowrap">${factura.subtotal.toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-xs sm:text-sm pb-2 border-b border-gray-200">
                <span className="text-gray-600">IVA (19%):</span>
                <span className="font-medium whitespace-nowrap">${factura.iva.toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-base sm:text-lg lg:text-xl font-bold pt-2 sm:pt-3 border-t-2 border-gray-300">
                ...
                <span className="text-blue-600 whitespace-nowrap">${factura.total.toLocaleString()}</span>
              </div>
            </div>
          </div>
          ...
          {factura.observaciones && (
            <div className="mt-6 sm:mt-8 p-4 bg-gray-50 rounded-lg">
              <h3 className="text-xs sm:text-sm font-bold text-gray-700 uppercase mb-2">Observaciones</h3>
              <p className="text-xs sm:text-sm text-gray-600 whitespace-pre-wrap break-words">{factura.observaciones}</p>
            </div>
          )}
          ...
          <div className="mt-8 sm:mt-12 pt-4 sm:pt-6 border-t border-gray-200 text-center text-xs sm:text-sm text-gray-600">
            ...
            <p className="mt-1">Para cualquier consulta, contáctenos al 317 060 7449</p>
          </div>
        </div>
      </div>
    </div>
