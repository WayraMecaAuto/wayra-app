...
export default function ReportesWayraTaller() {
  ...
  const [añoComparacion, setAñoComparacion] = useState(
  ...
  const canViewContabilidad = ["SUPER_USUARIO", "ADMIN_WAYRA_TALLER"].includes(
    session?.user?.role || ""
  );
  ...
  const cargarDatos = async () => {
    ...
    try {
      ...
      if (
        (vistaActual === "general" || vistaActual === "contabilidad") &&
        canViewContabilidad
      ) {
      ...
      if (vistaActual === "comparativa" && canViewContabilidad) {
        const res = await fetch(
          `/api/reportes/wayra-taller?tipo=comparativa&año=${año}&año2=${añoComparacion}`
        );
          ...
          <CardContent className="p-3 sm:p-4">
            <div className="flex flex-wrap gap-2">
              {[
                ...
                canViewContabilidad && "comparativa",
              ]
                .filter(Boolean)
                .map((vista) => (
                  <Button
                ...
                {canViewContabilidad && contabilidad && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                    ...
                    <Card className="bg-gradient-to-br from-blue-50 to-blue-100">
                      <CardHeader className="pb-2">
                        <CardTitle className="text-sm sm:text-base text-blue-700">
                          Utilidad
                        </CardTitle>
                      </CardHeader>
                      <CardContent>
                    ...
                    <Card className="bg-gradient-to-br from-purple-50 to-purple-100">
                      ...
                      <CardContent>
                        <div className="text-xl sm:text-2xl lg:text-3xl font-bold text-purple-800">
                          {serviciosFrecuencia?.masRealizados.reduce(
                            (sum: number, s: any) => sum + s.veces_realizado,
                            0
                          ) || 0}
                        </div>
                    ...
                    <Card>
                      ...
                      <CardContent>
                        <div className="h-64 sm:h-80">
                          <Bar
                            data={{
                              labels: serviciosFrecuencia.masRealizados
                                .slice(0, 5)
                                .map((s: any) =>
                                  s.descripcion.length > 20
                                    ? s.descripcion.substring(0, 20) + "..."
                                    : s.descripcion
                                ),
                              datasets: [
                                {
                                  label: "Veces",
                                  data: serviciosFrecuencia.masRealizados
                                    .slice(0, 5)
                                    .map((s: any) => s.veces_realizado),
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <div>
                        ...
                        <Dropdown
                          ...
                          onChange={(val) =>
                            setFiltroServicios(val as "todo" | "mes")
                          }
                          icon={<Calendar className="h-4 w-4" />}
                        />
                      </div>
                        ...
                        <Bar
                          data={{
                            labels: serviciosFrecuencia.masRealizados
                              .slice(0, 8)
                              .map((s: any) =>
                                s.descripcion.length > 15
                                  ? s.descripcion.substring(0, 15) + "..."
                                  : s.descripcion
                              ),
                            datasets: [
                              {
                      ...
                      <table className="w-full text-xs sm:text-sm">
                        <thead className="bg-gray-50">
                          <tr>
                            ...
                            <th className="text-right py-2 px-2 sm:py-3 sm:px-4">
                              Veces
                            </th>
                            {canViewContabilidad && (
                              <>
                                <th className="text-right py-2 px-2 sm:py-3 sm:px-4 hidden sm:table-cell">
                                  Ingreso
                                </th>
                                <th className="text-right py-2 px-2 sm:py-3 sm:px-4">
                                  Promedio
                        ...
                        <tbody>
                          {serviciosFrecuencia.masRealizados.map(
                            (s: any, i: number) => (
                              <tr key={i} className="border-b">
                                <td className="py-2 px-2 sm:py-3 sm:px-4">
                                  {s.descripcion}
                                </td>
                                <td className="py-2 px-2 sm:py-3 sm:px-4 text-right font-bold">
                                  {s.veces_realizado}
                                </td>
                                {canViewContabilidad && (
                                  <>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="h-64 sm:h-96">
                      <Bar
                        data={{
                          labels: mecanicoProductividad.productividad.map(
                            (m: any) => m.mecanico
                          ),
                          datasets: [
                            {
                              label: "Órdenes",
                              data: mecanicoProductividad.productividad.map(
                                (m: any) => m.totalOrdenes
                              ),
                              backgroundColor: "rgba(99, 102, 241, 0.8)",
                            },
                          ],
                      ...
                      <table className="w-full text-xs sm:text-sm">
                        <thead className="bg-gray-50">
                          <tr>
                            ...
                            {canViewContabilidad && (
                              <>
                                <th className="text-right py-2 px-2 sm:py-3 sm:px-4 hidden lg:table-cell">
                                  Ingresos
                                </th>
                                <th className="text-right py-2 px-2 sm:py-3 sm:px-4">
                                  $/Orden
                        ...
                        <tbody>
                          {mecanicoProductividad.productividad.map(
                            (m: any, i: number) => (
                              <tr key={i} className="border-b">
                                ...
                                {canViewContabilidad && (
                                  <>
                                    ...
                                    <td className="py-2 px-2 sm:py-3 sm:px-4 text-right">
                                      $
                                      {parseFloat(
                                        m.ingresoPromedioPorOrden
                                      ).toLocaleString()}
                                    </td>
                                  </>
                                )}
                                <td className="py-2 px-2 sm:py-3 sm:px-4 text-right">
                                  {m.tiempoPromedioHoras}h
                                </td>
                              </tr>
                            )
                          )}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    <Card>
                      ...
                      <CardContent>
                        <div className="text-center">
                          <p
                            className={`text-3xl sm:text-4xl font-bold ${parseFloat(comparativa.crecimiento.ingresos) >= 0 ? "text-green-600" : "text-red-600"}`}
                          >
                    ...
                    <Card>
                      ...
                      <CardContent>
                        <div className="text-center">
                          <p
                            className={`text-3xl sm:text-4xl font-bold ${parseFloat(comparativa.crecimiento.ordenes) >= 0 ? "text-green-600" : "text-red-600"}`}
                          >
                            {parseFloat(comparativa.crecimiento.ordenes) >= 0
                              ? "+"
                              : ""}
                          ...
                          <div className="mt-4 space-y-2 text-xs sm:text-sm">
                            <div className="flex justify-between">
                              <span>{comparativa.año1.año}:</span>
                              <span className="font-bold">
                                {comparativa.año1.totalOrdenes}
                              </span>
                            </div>
                            <div className="flex justify-between">
                              <span>{comparativa.año2.año}:</span>
                              <span className="font-bold">
                                {comparativa.año2.totalOrdenes}
                              </span>
                            </div>
                          </div>
                        </div>
