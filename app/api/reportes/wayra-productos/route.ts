...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log("📊 API Wayra Productos - Parámetros:", {
      tipo,
      periodo,
      año,
      mes,
    ...
    if (tipo === "productos-vendidos") {
      ...
      console.log("🔍 Filtrando productos Wayra:", {
        mesFilter,
        añoFilter,
        quincenaFilter,
      });
    ...
    if (tipo === "comparativa") {
      ...
      const procesarDatosAnio = async (anioTarget: number) => {
        ...
        for (let mesNum = 1; mesNum <= 12; mesNum++) {
          const { ingresos, egresos } = await obtenerContabilidadMes(
            mesNum,
            anioTarget
          );
          todosIngresos.push(...ingresos);
