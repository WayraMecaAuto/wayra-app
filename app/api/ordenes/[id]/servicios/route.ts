...
export async function POST(
  ...
  try {
    ...
    if (productosLubricacion && Array.isArray(productosLubricacion) && productosLubricacion.length > 0) {
      console.log(`🔧 Procesando ${productosLubricacion.length} productos de lubricación`)
      ...
      for (const prod of productosLubricacion) {
        ...
        if (producto.tipo === 'WAYRA_CALAN' && producto.monedaCompra === 'USD') {
          ...
          console.log(`💱 CALAN ${producto.nombre}: $${producto.precioCompra} USD → $${precioCompraCOP.toFixed(2)} COP`)
        }
        ...
        console.log(`   💰 Precio Compra (COP): $${precioCompraCOP.toFixed(2)}`)
        console.log(`   💰 Precio Minorista: $${producto.precioMinorista}`)
        console.log(`   💰 Utilidad: $${(producto.precioMinorista - precioCompraCOP).toFixed(2)}`)
      }
    }
    ...
    console.log(`   💰 Costo Productos (COP): $${costoTotalProductos.toFixed(2)}`)
    console.log(`   💰 Utilidad Wayra Taller: $${utilidadLubricacion.toFixed(2)}`)
