...
export async function GET(request: NextRequest) {
  try {
    ...
    const ingresos: any[] = [];
    const serviciosPorDescripcion: {
      [key: string]: { cantidad: number; ingresoTotal: number };
    } = {};
    ...
    ordenesCompletadas.forEach((orden) => {
      orden.servicios.forEach((servicio) => {
        ...
        let descripcionBase = servicio.descripcion.trim();
        if (
          descripcionBase.toLowerCase().includes("lubricación") ||
        ...
        serviciosPorDescripcion[descripcionBase].ingresoTotal +=
          servicio.precio;
      });
